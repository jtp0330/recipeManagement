...
public class JwtUtils {
	...
	public ResponseCookie generateJwtCookie(UserDetailsImpl userPrincipal) {
		...
		ResponseCookie cookie = ResponseCookie.from(jwtCookie, jwt).path("/recipes").maxAge(24 * 60 * 60).httpOnly(true)
				.build();
		return cookie;
	}

	public ResponseCookie getCleanJwtCookie() {
		ResponseCookie cookie = ResponseCookie.from(jwtCookie, null).path("/recipes").build();
		return cookie;
	}
